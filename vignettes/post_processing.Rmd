...
After your pipeline runs successfully via `execStudyPipeline()`, raw analytical outputs are saved in `exec/results/[database]/[version]/` organized by task. Post-processing orchestrates the merge and quality control of these results across multiple databases and exports them to `dissemination/export/merge/v{version}/`.

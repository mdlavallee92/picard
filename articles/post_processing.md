...
raw analytical outputs are saved in `exec/results/[database]/[version]/`
organized by task. Post-processing orchestrates the merge and quality
control of these results across multiple databases and exports them to
`dissemination/export/merge/v{version}/`.

The main post-processing step is **orchestrating the pipeline export**,
which:
...
to merge and validate all results:
...
- **Calls
  ...
  for each task to merge results across databases
...
Maps cohort IDs to labels for interpretation. Pulled from your cohort
...
Inspects the structure of all exported CSV files. Useful for
identifying:
...
Validates that all cohorts in your cohort key have corresponding
results. Flags:

- **OK:** Cohort found in results with non-zero counts
- **ZeroCount:** Cohort found but has zero entries or subjects
  (non-enumerated)
